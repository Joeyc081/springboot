...
import cn.soft.market_management.dao.SellDao;
...
public class SellServiceImpl implements SellService {
    ...
    public int saveSell(Sell sell) {
        int count=sellDao.saveSell(sell);
        if (!(count>0))
            throw new ServiceException("插入失败");
        return count;
    }
    @RequiredLog("分页查询操作")
    public PageObject<Sell> findPageObjects(String pname, Integer pageCurrent) throws IllegalAccessException {
        if (pageCurrent==null||pageCurrent<1)
            throw new IllegalAccessException("当前页码不正确");

        //基于条件 进行总记录查询
        int rowCount =sellDao.getRowCount(pname);
        if (rowCount==0)
            throw new SecurityException("系统没有查到对应记录");
        int pageSize=10;
        int startIndex=(pageCurrent-1)*pageSize;
        //执行查询当前页操作
        List<Sell> records=sellDao.findPageObjects(pname,startIndex,pageSize);
        PageObject<Sell> pageObject=new PageObject<>(pageCurrent,pageSize,rowCount,records);
        return pageObject;
